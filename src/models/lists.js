...
const listRepository = {
  ...
  async findAll() {
    const result = await List.find({}).populate('songs').populate('user', 'usuario').exec();
    return result;
  },

  async findAllByUser(id_user) {
    const result = await List.find({}).where('user').equals(id_user).populate('songs').populate('user', 'usuario').exec();
    return result;
  },
